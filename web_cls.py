...
class WebAdd(QtWidgets.QDialog):
    ...
    def resizeEvent(self, a0: QtGui.QResizeEvent) -> None:
        ...
        self.ui.line_2.move(self.ui.line_2.pos().x(), 330*scale)
        # Labels
        self.ui.lbl_caption.move(self.ui.lbl_caption.pos().x(), 85*scale)
        self.ui.lbl_web_page.move(self.ui.lbl_web_page.pos().x(), 125*scale)
        self.ui.lbl_description.move(self.ui.lbl_description.pos().x(), 165*scale)
        self.ui.lbl_username.move(self.ui.lbl_username.pos().x(), 260*scale)
        self.ui.lbl_password.move(self.ui.lbl_password.pos().x(), 295*scale)
        # LineEdit boxes
        self.ui.txt_caption.move(self.ui.txt_caption.pos().x(), 80*scale)
        self.ui.txt_caption.resize(w-220, self.ui.txt_caption.height())
        self.ui.txt_web_page.move(self.ui.txt_web_page.pos().x(), 120*scale)
        self.ui.txt_web_page.resize(w-220, self.ui.txt_web_page.height())
        self.ui.txt_description.move(self.ui.txt_description.pos().x(), 160*scale)
        self.ui.txt_description.resize(w-220, self.ui.txt_description.height())
        self.ui.txt_username.move(self.ui.txt_username.pos().x(), 255*scale)
        self.ui.txt_username.resize(w-490, self.ui.txt_username.height())
        self.ui.txt_password.move(self.ui.txt_password.pos().x(), 290*scale)
        self.ui.txt_password.resize(w-490, self.ui.txt_password.height())
        # Buttons
        self.ui.btn_show_password.move(w-280, 290*scale)
